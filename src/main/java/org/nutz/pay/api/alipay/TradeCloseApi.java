...
import org.nutz.pay.bean.alipay.req.unity.TradeClose;
import org.nutz.pay.bean.alipay.resp.unity.TradeCloseResp;
...
/**
 * <a href="https://doc.open.alipay.com/docs/api.htm?spm=a219a.7395905.0.0.UGwRdC&docType=4&apiId=1058">统一收单交易关闭接口</a>
 * alipay.trade.close
 * Created by Jianghao on 2017/4/22
 *
 * @howechiang
 */
public class TradeCloseApi {
    ...
    public static String checkParams(Base req) {
        ...
        if (Strings.isEmpty(req.getApp_id())) {
            ...
            return "接口名称不能为空";
        } else if (!Strings.equals(req.getMethod(), "alipay.trade.close")) {
            return "接口名称错误, 应该为alipay.trade.close";
            ...
            TradeClose tc = Lang.map2Object(req.getBiz_content(), TradeClose.class);
    ...
    public static String getSign(Base req, String k) {
        ...
        if (Strings.isEmpty(checkParams(req))) {
            ...
            if (Strings.equalsIgnoreCase(req.getSign_type(), "RSA2")
                ...
                log.error("支付宝统一收单交易关闭接口签名方式只支持RSA、RSA2.");
                return null;
            }
        } else {
            return null;
