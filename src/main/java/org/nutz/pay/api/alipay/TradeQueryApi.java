...
import org.nutz.pay.bean.alipay.req.wap.TradeCancel;
import org.nutz.pay.bean.alipay.resp.wap.TradeCancelResp;
...
/**
 * <a href="https://doc.open.alipay.com/docs/api.htm?spm=a219a.7395905.0.0.qKcgjR&docType=4&apiId=757">统一收单线下交易查询接口</a>
    ...
    public static String getSign(Base req, String k) {
        ...
        if (Strings.isEmpty(checkParams(req))) {
            ...
            if (Strings.equalsIgnoreCase(req.getSign_type(), "RSA2")
                ...
                log.error("支付宝统一收单线下交易查询接口签名方式只支持RSA、RSA2.");
                return null;
            }
        } else {
            return null;
